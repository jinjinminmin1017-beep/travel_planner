...
- [ ] 按设计稿重构 `PlanningScreen`。
- [ ] 将真实起终点和进度传入组件。
- [ ] 实现四阶段状态和地图高亮。
- [ ] 保留取消异步任务功能。
- [ ] 补充 reduced motion 处理。

### Phase 3：方案总览

- [ ] 实现 `RouteSummaryHero`。
- [ ] 用 `PlanSelector` 替换横向滚动推荐卡。
- [ ] 实现真实 `RecommendationRationale`。
- [ ] 实现 `RouteTimeline`。
- [ ] 增加固定底部收藏和详情入口。
- [ ] 保留数据状态、调整时间和来源入口。
...
- [ ] 扩展 `resultsPane` 状态。
- [ ] 实现 `RouteDetailScreen` 和 `JourneyLegCard`。
- [ ] 把当前 `DetailPanel` 能力按信息层级迁移到详情页。
- [ ] 保留座席、舱位、接驳方案重新计算。
- [ ] 保留外部预订跳转、分享、复制和反馈。
- [ ] 风险未绑定交通段时统一显示为方案级提示。
...
- [ ] 完成 loading、skeleton、empty、error、partial、disabled、selected、pressed、expanded 状态。
- [ ] 验证安全区、文本放大、横竖屏和 Web 居中。
- [ ] 检查触控目标、颜色对比和读屏文案。
...
- [ ] `npm run typecheck`
- [ ] `npm run build`
- [ ] 在 360、390、430px 手机宽度完成视觉回归。
- [ ] 在 Android 和 iOS 至少各验证一次安全区和固定底栏。
- [ ] 验证无方案、部分结果、来源失败、重新规划和取消任务。
- [ ] 验证现有埋点事件名称没有改变。
...
- [ ] 390×844 下与已确认设计稿的结构和层级一致。
- [ ] 首屏可看到路线结论、三个核心指标、方案选择和时间轴开头。
- [ ] 卡片圆角不超过 16px，控件圆角统一为 12px。
- [ ] 全页只使用青色作为主强调色，警示色仅用于真实风险。
- [ ] 不存在嵌套卡片造成的重复边框和阴影。
- [ ] 按钮文字不换行，正文与背景满足 WCAG AA 对比。

### 11.2 交互

- [ ] 三个推荐方案可切换，选中状态明确。
- [ ] 方案切换同步更新摘要、推荐理由、时间轴和详情。
- [ ] 收藏、分享、复制、调整、预订跳转、反馈、来源查看均可用。
- [ ] 总览、详情、来源之间返回时不丢失当前方案。
- [ ] 所有按钮点击热区不小于 44×44px。
- [ ] 读屏能识别控件名称、选中、禁用和展开状态。

### 11.3 数据

- [ ] 页面不显示硬编码示例价格、时间或车次。
- [ ] 所有比较结论基于当前响应中的真实方案计算。
- [ ] 缺失字段有自然降级文案，不显示 `undefined`、`null` 或原始状态码。
- [ ] 风险没有明确交通段关联时不进行猜测映射。
- [ ] 数据源失败、授权边界和降级信息保持可访问。

### 11.4 工程质量

- [ ] TypeScript 类型检查通过。
- [ ] Expo 导出构建通过。
- [ ] 新组件职责清晰，`App.tsx` 的路径规划展示逻辑明显减小。
- [ ] 没有新增不必要依赖、手绘 SVG 或逐帧 React state 动画。
- [ ] 动画有 reduced motion 降级，不阻塞内容显示。
