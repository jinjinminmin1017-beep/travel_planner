...
| LLM Intent Parser | 验收通过 | 已有 Prompt 模板、版本常量、真实 LLM wrapper、一次 repair、语义校验、规则 fallback、LLM 调用审计日志和缺信息追问。 | `real_llm` 默认禁用时使用规则 fallback；真实 LLM 仍需用户自有 key 与授权后才能启用。 |
...
| 铁路规划 | 动态直达运行时已替换旧方案 / 中转票源增强待动态化 | Planner 已可按地点解析出的候选站点生成站点对查询，并仅用 `rail_authorized_partner` 返回的真实车次、票价、余票组装直达铁路方案；旧 `rail_planning_engine` 仅保留中转、多段中转、票源增强 S/A/NOT_RECOMMENDED/BLOCKED 与安全阻断的独立测试覆盖，不再参与 `build_plans()` 运行时结果；`rail_12306_redirect` 可跳转官方入口。 | 中转、多段和票源增强仍未动态生成，当前以能力缺口阻断且不再用旧模板补造；`rail_authorized_partner` 未授权前不得展示真实中国铁路票价、余票或可售承诺；测试 fixture 不得作为运行时 fallback。 |
| 航班规划 | 验收通过 / 真实报价阻塞于授权 | `flight_planning_engine` 已覆盖直飞、中转、多机场组合；Planner 使用 Amadeus Offers 搜索并要求 Price 确认后才生成航班段；OpenSky/天气可作为风险辅助边界。 | `amadeus_flight_offers`、`amadeus_flight_price` 缺用户自有凭证前不得生成真实报价、可售状态或舱位承诺；fixture 不得作为运行时 fallback。 |
...
| LLM Recommendation | 验收通过 / 真实 LLM 阻塞于授权 | Recommendation Prompt、语义校验 REC-001 至 REC-012、一次 Repair Prompt、LLMValidationResult 元数据和候选池外/不可选/BLOCKED/过期方案拒绝链路已完成；LLM 不可用时返回 `recommendation_result = null`，候选仍可展示。 | `real_llm` 缺用户自有 key 和授权前不得宣称真实 LLM 推荐上线；不得用代码 fallback 生成三张推荐卡。 |
...
| Redirect-only Handoff | 验收通过 | 12306、航司官网、OTA 合作模板、高德/百度地图与打车导航入口均走 redirect-only；每个 redirect 带 generated_at、expires_at、transaction_boundary、DataSourceMetadata，URL 安全检查会阻断登录/下单/支付/抢票参数并返回 fallback_instruction。 | OTA 和打车平台深度能力仍受合作方模板与外部 App 授权限制；只跳转，不登录、不下单、不支付。 |
...
| 方案详情可信解释 | 验收通过 | App 详情页展示时间线、费用明细、估算标记、风险提示、舒适度拆解、数据完整度/置信度、最新数据更新时间、数据来源、票源增强等级与航班中转风险边界。 | 只解释已由后端结构化输出或授权数据源提供的事实；不承诺有票、可售、下单成功或第三方平台实时状态。 |
...
|---|---|---|---|---|---|
| `internal_calc` | INTERNAL_CALCULATION | 验收通过 | 启用 | 确定性费用汇总、风险汇总、规则降级说明和接驳估算。 | 伪装成真实 Provider 返回票价、余票、航班或第三方路线事实。 |
...
| `irail_connections` | RAIL | 验收通过 | 启用 | 公开铁路连接 smoke，证明真实铁路 Provider 链路。 | 代表中国铁路票价、余票或 12306 可售能力。 |
...
| `amadeus_flight_offers` | FLIGHT | 阻塞于授权 | 禁用 | 用户自有 Amadeus 凭证存在时做航班报价搜索。 | 无凭证时生成真实报价或可售承诺。 |
| `amadeus_flight_price` | FLIGHT | 阻塞于授权 | 禁用 | 用户自有 Amadeus 凭证存在时做报价校验。 | 无凭证时生成价格确认结果。 |
| `variflight_status` | FLIGHT | 阻塞于授权 | 禁用 | 授权后提供商业航班状态/延误风险。 | 未授权时调用或把 OpenSky 结果冒充商业状态。 |
| `rail_authorized_partner` | RAIL | 聚合数据 817 Adapter 已接入 / 阻塞于授权 | 禁用 | 配置聚合数据 `https://apis.juhe.cn/fapigw/train/query` 和用户自有 key 后，提供铁路时刻、票价、余票只读搜索。 | 逆向 12306、抢票、占票、下单、支付。 |
| `ota_partner_redirect` | OTA | 阻塞于授权 | 禁用 | 合作方模板和参数规范确认后做 redirect-only handoff。 | 未合作时拼接 OTA 交易链接或携带下单参数。 |
...
| `backend/app/tests/*_providers.py` | 自动化专用 | 使用 fake HTTP client 映射 Provider 响应结构。 | 只验证 Adapter 解析逻辑，不证明第三方服务实时可用。 |
| `frontend/ui-preview.html` 与 `frontend/ui-preview-screenshot.png` | 自动化专用 | App 输入、规划中、结果详情、数据来源、错误和空结果状态的视觉预览。 | 不作为运行时数据来源，不代表真实规划结果或第三方平台实时可用。 |
...
| 路线或场景 | 当前覆盖 | 真实能力边界 |
|---|---|---|
| 上海嘉定南翔格林公馆 -> 青岛金水假日酒店 | 运行时覆盖动态铁路直达和接驳切换；铁路中转、多段中转、航班直飞、航班中转、航班多机场、航铁混合、票源增强当前返回能力缺口说明，不再输出旧模板方案。 | 地点/节点由 Location Resolver 管理；真实铁路票价/余票依赖 `rail_authorized_partner` 授权，真实航班报价依赖后续动态航班 Planner 接入。 |
...
| 航班段 | 可在测试 fixture 下覆盖直飞、中转、多机场组合、Amadeus Offers/Price 确认链路和阻断语义。 | 无 Amadeus 凭证和授权时不得承诺真实报价、可售状态或舱位；Price 确认失败时不得生成对应航班方案。 |
| 铁路段 | 可在测试 fixture 下覆盖车次、座席、动态中转站、多段中转、票源增强和阻断语义。 | 未取得授权前不得承诺中国铁路票价、余票、可售状态或补票成功。 |
| Redirect-only | 覆盖 12306、航司官网、高德地图跳转和 fallback instruction。 | 只跳转，不登录、不抢票、不下单、不支付、不保存第三方凭证。 |

当前可以对外演示的是“可解释的规划骨架 + 地点/节点候选 + 真实公开只读 Provider smoke + 授权缺失时的透明降级/阻断”。不能对外演示或承诺“任意城市实时最优路线、真实票价余票、自动购票、抢票、支付、第三方账号登录、LLM 生成交通事实”。
