...
- Safety-gate smoke: `python scripts/continuous_flight_smoke.py --mode gate --iterations 3 --interval-seconds 0`
  - Result: 3/3 passed.
  - Each iteration confirmed all 10 registered source systems remained `DISABLED`, `PENDING_REVIEW`, and blocked by their own contract.
...
- Secret-tier configuration check: expected failure for all disabled providers.
