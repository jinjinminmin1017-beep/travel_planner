...
- `frontend/src/components/`：规划状态、正常结果、路线详情和约束无匹配页面组件。
  - `frontend/src/components/input/TravelInputScreen.tsx`：Approved V2 自然语言输入、快捷偏好、脱敏历史与留存偏好面板。
  - `frontend/src/components/results/JourneyLegCard.tsx`、`JourneyCostSummary.tsx`：连续分段路线、可靠单段费用、席别/舱位/接驳调整和权威总价。
  - `frontend/src/components/results/TransportModeSelector.tsx`：铁路/航班独立入口；只消费真实计划与既有失败/缺失说明，不生成占位交通事实。
...
- `backend/app/services/planning_progress.py`：不依赖 HTTP、SQLite 或全局 store 的渐进结果协议、线程安全单计划候选协调器与结构化阶段指标载体。
- `backend/app/services/task_queue.py`：异步任务期限、Provider 超时、并发和渐进结果开关；期限默认处于观测模式。
- `backend/app/services/local_transfer_engine.py`：规划请求内地点解析与地图路线复用；支持 selected 接驳先验证发布、alternatives 后补齐，以及机场/远距离 walking 安全短路。
...
- `backend/app/data_sources/`：地图、地理编码、铁路、航班、天气、LLM、跳转和数据源配置适配器。
  ...
  - `flight_evidence_store.py`：航司请求/响应完整脱敏交换证据；SQLite/WAL、压缩正文、原始/脱敏哈希、required fail-closed、导出敏感扫描与分级保留清理。
...
- 航司完整交换证据追加写入同一 SQLite 的 `flight_provider_exchanges` 表；`scripts/export_flight_exchange.py` 负责受控导出与过期清理，不提供公网下载接口。
