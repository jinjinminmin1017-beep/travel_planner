...
状态：已完成。完成时间：2026-07-22 23:22:49 +08:00。代码提交：`85a82f4c1325f2f9a83a699cdfdd7f57b5980a66`。

实现结果：

- 航班聚合器现按来源返回结构化 outcome，分别保留 429、空结果、超时、挑战、非法响应、禁用和成功结果；真实 offer 与其他来源失败可以同时保留。
- Planner 按请求交通方式范围决定查询和缺失语义；显式排除航班时不查询、不报告失败，航班暂不可确认且铁路可用时返回 `PARTIAL`，所有航班来源确认空结果时不误报系统失败。
- 前端补齐 `missing_plan_explanations` 与 `SourceFailure` 类型，新增独立铁路/航班入口、真实候选筛选、不可用原因展开和受防重复提交保护的来源重试。
- 前端功能开关：`EXPO_PUBLIC_TRANSPORT_MODE_SELECTOR_ENABLED`，默认启用；关闭后保留原推荐目标选择界面。
- 验证：后端全量 `235 passed`；前端 helper/UI 合同 `14 passed`；TypeScript 检查与 Expo Web/iOS/Android 导出通过。
