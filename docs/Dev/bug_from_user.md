...
## 2026-08-07 铁路发现摘要少计停站导致合法详情无法入库

- 用户提问时间：2026-08-07。
- 问题描述：8 月 10 日详情导入完成 5069 趟后，D8813 因发现摘要 `total_num=4`、详情返回 5 行而触发 `extra stops cannot be reconciled unambiguously`；resume 在同一车次确定性复现。
- 问题根因：D8813 的 5 行全部标识为同一车次，首末站为哈密、乌鲁木齐，按首站 20:20 对照各行 `running_time` 的累计偏差仅 2～6 分钟。详情是完整一致的 5 站服务，错误来自发现摘要少计一站；旧规则只识别“详情混入异常父交路行”，要求删行证据，无法表达发现计数偏小。
- 解决方式：仅对“恰好多一站”的详情增加保守接受路径，并同时要求所有行车次号等于发现车次、首末站与发现一致、所有累计运行时偏差不超过 15 分钟。任一条件不满足则继续使用原异常行对账或失败关闭。
- 问题修改提交：`bb478c5`。
- 验证：新增 D8813 同型回归，5 个停站全部保留且站序连续；既有 C824 异常行剔除回归继续通过；铁路定向测试 21 passed，后端全量测试 298 passed，Python compileall 与 `git diff --check` 通过。

## 2026-08-08 当前官方目录缺少玉环站 telecode 阻塞服务日导入

- 用户提问时间：2026-08-08。
- 问题描述：8 月 11 日详情导入完成 8728 趟后，G7364（玉环至上海南）首站“玉环”无法映射 telecode，批次按门禁进入 FAILED。
- 问题根因：G7364 当前官方精确发现确认始发站名为“玉环”，但不携带站码；当前官方 `station_name.js` 的 3397 站与官方全站起售目录的 3169 站均无该站，站点精确搜索又触发访问控制。项目不能从拼音猜测 telecode，也不能写入空站码。
