...
def _minutes_between(actual: TimePoint, requested: TimePoint) -> int:
    return max(0, ceil(abs((actual.datetime - requested.datetime).total_seconds()) / 60))
...
def evaluate_time_constraints(plan: TravelPlan, request: TravelRequest) -> tuple[list[ConstraintViolation], list[ConstraintType]]:
    ...
    if earliest and departure:
        if departure.datetime < earliest.datetime:
    ...
    if latest and arrival:
        if arrival.datetime > latest.datetime:
    ...
    if request.time_anchor_type == "ARRIVAL" and arrival:
        if request.time_window_start and arrival.datetime < request.time_window_start.datetime:
        ...
        elif request.time_window_end and arrival.datetime > request.time_window_end.datetime and not latest:
        ...
        if request.time_window_start and departure.datetime < request.time_window_start.datetime and not earliest:
        ...
        elif request.time_window_end and departure.datetime > request.time_window_end.datetime:
