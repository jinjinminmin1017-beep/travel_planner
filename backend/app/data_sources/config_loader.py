...
def load_flight_evidence_config(
    ...
    *,
    ...
    backend_value = values.get("TRAVEL_FLIGHT_EVIDENCE_BACKEND", "sqlite").strip().lower()
    if backend_value not in {"sqlite", "disabled"}:
        raise DataSourceConfigurationError(
            "invalid key TRAVEL_FLIGHT_EVIDENCE_BACKEND"
        )
    required_default = env == "PROD"
    required = _parse_bool(
        "flight_evidence",
        "REQUIRED",
        values.get(
            "TRAVEL_FLIGHT_EVIDENCE_REQUIRED",
            "true" if required_default else "false",
        ),
    )
    hmac_value = values.get("TRAVEL_FLIGHT_EVIDENCE_HMAC_KEY", "")
    if required and not hmac_value:
        raise DataSourceConfigurationError(
            "missing key TRAVEL_FLIGHT_EVIDENCE_HMAC_KEY"
        )
    try:
        config = FlightEvidenceConfig(
            backend=backend_value,
            path=Path(
                values.get(
                    "TRAVEL_FLIGHT_EVIDENCE_PATH",
                    "logs/flight_harvest.sqlite3",
                )
            ),
            required=required,
            hmac_key=hmac_value.encode("utf-8") if hmac_value else None,
            success_retention_days=_parse_int(
                "flight_evidence",
                "SUCCESS_RETENTION_DAYS",
                values.get(
                    "TRAVEL_FLIGHT_EVIDENCE_SUCCESS_RETENTION_DAYS",
                    "90",
                ),
                minimum=1,
            ),
            failure_retention_days=_parse_int(
                "flight_evidence",
                "FAILURE_RETENTION_DAYS",
                values.get(
                    "TRAVEL_FLIGHT_EVIDENCE_FAILURE_RETENTION_DAYS",
                    "180",
                ),
                minimum=1,
            ),
        ...
        config.validate()
        return config
    except FlightEvidencePersistenceError as exc:
        raise DataSourceConfigurationError(
            "invalid flight evidence configuration"
        ) from exc
